...
use crate::{
    BinaryOperator, CollectiveOperation, DerivativeAxis as Axis, DerivativeControl as Ctrl,
    Expression, GatherMode, Handle, ImageClass, ImageDimension as Dim, ImageQuery, MathFunction,
    Module, RelationalFunction, SampleLevel, Scalar, ScalarKind as Sk, Span, SubgroupOperation,
        ...
        | "textureProjOffset" => {
            let f = |kind, dim, arrayed, multi, shadow| {
                for bits in 0..=0b11 {
                    ...
                    if !shadow && !proj && kind == Sk::Float && !multi && dim != Dim::D3 {
                        let mut depth_args = args.clone();
                        depth_args[0] = TypeInner::Image {
                            dim,
                            arrayed,
                        ...
                        declaration
                            .overloads
                            .push(module.add_builtin(depth_args, builtin));
                    }
                    declaration
                        .overloads
        ...
        match sampler {
            ...
            | "sampler2DMSArray" | "sampler3D" | "samplerCube" | "samplerCubeArray" => {
                let dim = match sampler {
                    ...
                    "sampler2D" | "sampler2DArray" | "sampler2DMS" | "sampler2DMSArray" => Dim::D2,
                    "sampler3D" => Dim::D3,
                    _ => Dim::Cube,
                };
                let arrayed = matches!(
                ...
                let multisampled = matches!(sampler, "sampler2DMS" | "sampler2DMSArray");
                declaration.overloads.push(module.add_builtin(
                    vec![
                        TypeInner::Image {
                            dim,
                ...
                // A separate GLSL texture object can be used with both a
                // comparison sampler and a regular sampler. SamplerShadow
                // promotes the image to Depth, so retain the ordinary sampler
                // constructor overload for that inferred class as well.
                if kind == Sk::Float && !multisampled && dim != Dim::D3 {
                    declaration.overloads.push(module.add_builtin(
                        vec![
                            TypeInner::Image {
                                dim,
    ...
    match name {
        ...
        "pow" => {
            ...
            for size in [
                ...
                let ty = || match size {
                    ...
                    None => TypeInner::Scalar(Scalar::F16),
                };
                declaration.overloads.push(
                    module
                        .add_builtin(vec![ty(), ty()], MacroCall::MathFunction(MathFunction::Pow)),
        ...
        "mix" => {
            ...
            for size in [
                ...
                declaration.overloads.push(module.add_builtin(
                    vec![ty(Scalar::F16), ty(Scalar::F16), TypeInner::Scalar(Scalar::F16)],
                    MacroCall::Splatted(MathFunction::Mix, size, 2),
                ));
                if size.is_some() {
                    declaration.overloads.push(module.add_builtin(
                        vec![ty(Scalar::F16), ty(Scalar::F16), ty(Scalar::F16)],
                        MacroCall::Splatted(MathFunction::Mix, size, 2),
                    ));
                }
            }
        ...
        _ => {
            inject_common_builtin(declaration, module, name, 4);
            inject_common_builtin(declaration, module, name, 2);
        }
    }
}
...
fn inject_common_builtin(
    ...
    match name {
        ...
        "subgroupMin" | "subgroupMax" | "subgroupOr" | "subgroupBroadcastFirst" => {
            let builtin = match name {
                "subgroupMin" => MacroCall::SubgroupCollective(SubgroupOperation::Min),
                "subgroupMax" => MacroCall::SubgroupCollective(SubgroupOperation::Max),
                "subgroupOr" => MacroCall::SubgroupCollective(SubgroupOperation::Or),
                "subgroupBroadcastFirst" => MacroCall::SubgroupBroadcastFirst,
                _ => unreachable!(),
            };
            declaration
                .overloads
                .push(module.add_builtin(vec![TypeInner::Scalar(Scalar::U32)], builtin));
        }
    ...
    pub fn call(
        ...
        Ok(Some(match *self {
            ...
            MacroCall::Texture {
                ...
                {
                    if let SampleLevel::Exact(ref mut expression) = level {
                        ctx.conversion(expression, meta, Scalar::I32)?;
                    }
                }
            ...
            MacroCall::SubgroupCollective(op) => {
                let result_type = ctx.resolve_type(args[0], meta)?.clone();
                let ty = ctx.module.types.insert(
                    Type {
                        name: None,
                        inner: result_type,
                ...
                let result =
                    ctx.interrupt_emitter(Expression::SubgroupOperationResult { ty }, meta);
                ctx.body.push(
            ...
            MacroCall::SubgroupBroadcastFirst => {
                let result_type = ctx.resolve_type(args[0], meta)?.clone();
                let ty = ctx.module.types.insert(
                    Type {
                        name: None,
