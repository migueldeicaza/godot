            ...
            crate::TypeInner::BindingArray { base, size } => {
                ...
                match size {
                    crate::ArraySize::Constant(size) => {
                        write!(out, "{NAMESPACE}::array<{base_tyname}, {}>", size.get())
                    }
                    crate::ArraySize::Pending(_) | crate::ArraySize::Dynamic => write!(
                        out,
...
{level}{loop_bound_name} -= uint2({loop_bound_name}.y == 0u, 1u);"
    ...
    fn put_subscripted_access_chain(
        ...
        let accessing_wrapped_binding_array = matches!(
            *base_ty,
                        ...
                        | crate::AddressSpace::Handle => {
                            ...
                            let good = match target {
                                Some(target) => match module.types[var.ty].inner {
                                    ...
                                    crate::TypeInner::BindingArray { base, size } => {
                                        match (&module.types[base].inner, size) {
                                            (
                                                crate::TypeInner::Image { .. },
